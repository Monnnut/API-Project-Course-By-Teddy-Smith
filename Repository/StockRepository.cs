...
namespace api.Repository
{
    public class StockRepository : IStockRepository
    {
        ...
        public async Task<List<Stock>> GetAllAsync(QueryObject query)
        {
            ...
            //include tells us the related Comments for each stocks
            // converts result into Querable obkect, which means it can be modify
            var stocks = _context.Stocks.Include(c => c.Comments).ThenInclude(a => a.AppUser).AsQueryable();
            if (!string.IsNullOrWhiteSpace(query.CompanyName))
            {
                //where helps to filter
                stocks = stocks.Where(s => s.CompanyName.Contains(query.CompanyName));
            }

            if (!string.IsNullOrWhiteSpace(query.Symbol)) //check if string query.Symbol has some meaningful content
            {
            ...
            //Pagination. To break down big data into smaller size
